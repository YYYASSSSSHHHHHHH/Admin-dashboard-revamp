...
const NAV = [
  ...
  { href: "/plans", label: "Membership Plans", icon: Package },
];
...
const Item = ({ href, label, icon: Icon, disabled }: { href: string; label: string; icon: any; disabled?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-3 py-4 flex flex-col gap-1 overflow-y-auto">
        <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500 px-3 mb-2">
          Workspace
        </div>
        {NAV.map((item) => (
          <Item
        ...
        <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500 px-3 mt-6 mb-2">
          System
        </div>
        {SECONDARY.map((item) => (
          <Item
